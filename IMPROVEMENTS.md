...
- **Règle** : L'historique des migrations est un programme qui doit s'exécuter du premier au dernier fichier sur une base vierge. Toute nouvelle migration s'écrit sous forme idempotente et auto-suffisante : `CREATE TABLE IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS`, `ADD COLUMN IF NOT EXISTS`, `DROP CONSTRAINT IF EXISTS`, et pour les policies et triggers soit un `DROP ... IF EXISTS` préalable, soit un `DO $do$ ... EXCEPTION WHEN duplicate_object THEN NULL; END $do$`. Ne jamais dépendre d'un objet créé par une migration postérieure ni d'un objet créé hors migration : si une table a été faite à la main en base, écrire son `CREATE TABLE IF NOT EXISTS` dans la migration qui l'utilise en premier. `CREATE OR REPLACE FUNCTION` ne peut ni changer le type de retour ni retirer des valeurs par défaut : faire précéder d'un `DROP FUNCTION IF EXISTS` avec la signature exacte. Ne jamais renommer ni réordonner une migration déjà appliquée : la production la connaît par son nom de version et ne la rejouera pas — corriger son contenu est sans risque, la déplacer ne l'est pas.
- **Vérification** : `bash scripts/replay-migrations.sh --all --test` doit afficher « migrations en échec : 0 » puis les tests pgTAP au vert. Le script rejoue les migrations sur un PostgreSQL local sans Docker, en une trentaine de secondes. Check [042] de check-rules.sh en garde-fou statique sur les migrations récentes (pas de `CREATE TABLE`/`CREATE INDEX`/`ADD COLUMN` sans `IF NOT EXISTS`, pas de `CREATE POLICY` sans garde dans le même fichier), et job CI `rls-tests.yml` sur toute PR touchant `supabase/migrations/`.
- **Fichiers de référence** : `scripts/replay-migrations.sh`, `scripts/replay-bootstrap.sql`, `.github/workflows/rls-tests.yml`, `supabase/migrations/20260728140000_mission_page_comments.sql` (forme idempotente complète), `supabase/migrations/20260623120000_add_newsletter_comments.sql` (convention `duplicate_object`)
