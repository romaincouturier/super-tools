...
- **Fichiers de référence** : `supabase/migrations/20260804160001_tender_ai_and_documents.sql` (bucket privé, policy storage réservée au staff, motif écrit dans le fichier), `src/hooks/useEntityDocuments.ts` (`resolveEntityDocumentUrl`), `supabase/functions/upload-tender-document/index.ts`
...
### [049] Événement de coût — idempotent par unité facturée, jamais par appel observé
- **Constat** : 04/08/2026, quelques heures après la mise en service du suivi des coûts. L'onglet Monitoring affichait pour `google-drive-helper` six transcriptions à 0,90 / 0,76 / 0,30 / 0,30 / 0,21 / 0,21 $ — les montants 0,30 et 0,21 apparaissant deux fois, à une heure d'intervalle. Ce n'étaient pas six transcriptions mais deux, comptées deux fois. Cause : `pollAssemblyAIJob` avait été instrumenté alors que c'est une **lecture** (un `GET` sur le transcript, gratuit), là où AssemblyAI facture une fois, à la soumission du job. Chaque relecture d'un job déjà terminé réécrivait un événement facturé pour la durée complète de l'audio — et le même job est relu au moins deux fois par construction, le webhook de fin et le cron `poll-drive-transcripts` interrogeant le même identifiant. Aucun test ne couvrait le cas « la même ressource est relue » : tous vérifiaient qu'un événement était écrit, jamais qu'il ne l'était qu'une fois. Le bug a été vu à l'œil nu sur des montants qui se répétaient, pas par le harnais.
- **Règle** : Un événement de coût représente **une unité facturée par le provider**, pas un appel sortant observé depuis le code. Avant d'instrumenter, identifier ce que le provider facture et à quel moment : à l'appel (Anthropic, OpenAI — un log par appel est correct), ou à la ressource (AssemblyAI facture le job de transcription, pas ses relectures). Dès que la ressource facturée peut être lue plusieurs fois, renseigner `externalId` avec son identifiant chez le provider : l'index unique partiel `(provider, external_id)` rejette alors le doublon, et le code `23505` doit être avalé silencieusement — c'est la déduplication qui fonctionne, pas une erreur. Corollaire de test : un test qui vérifie « un événement est écrit » ne suffit pas, il faut aussi vérifier « et un seul », en rejouant l'observation.
- **Vérification** : check [049] de `check-rules.sh` — tout fichier appelant `logAssemblyAiUsage(` doit mentionner `transcriptId` au moins autant de fois, et l'index unique `api_usage_events_external_id_uniq` doit exister dans les migrations. Contrôle en base : `SELECT external_id, count(*) FROM api_usage_events WHERE external_id IS NOT NULL GROUP BY external_id HAVING count(*) > 1` doit renvoyer zéro ligne.
- **Fichiers de référence** : `supabase/functions/_shared/api-usage.ts` (`externalId`, gestion du 23505), `supabase/migrations/20260804160500_api_usage_external_id.sql`, `supabase/functions/_shared/google-drive-helper.ts`
- **Origine** : constat user devant le top des appels — « c'est quoi ce délire » ; six lignes de transcription dont deux paires strictement identiques
- **Date** : 2026-08-04

### [050] Connecteurs paginés — le parcours des pages est une fonction testable, et une page en échec ne passe jamais pour un succès
- **Constat** : 05/08/2026, ajout de la source TED au module marchés publics. Le connecteur BOAMP lit tout en une requête ; le TED paginait par jeton (`iterationNextToken`). La première version gardait cette boucle DANS le handler edge, non testée — les 13 tests portaient sur le mapper et la construction de la requête, jamais sur le parcours. Pire, la boucle avalait les erreurs : un `break` sur une page en échec, puis un retour en succès. Une synchronisation à laquelle il manquait la moitié du flux se serait déclarée verte tous les matins, et comme l'erreur n'atteignait jamais `createErrorResponse`, Sentry n'aurait rien vu. Défaut jumeau découvert en relisant : la première page, déjà récupérée par le handler pour la sonde, était redemandée par la boucle — un appel payé deux fois, et surtout un second gel d'index en mode itération, donc un jeton qui ne correspondait plus au parcours en cours.
- **Règle** : Tout connecteur qui parcourt plusieurs pages met la BOUCLE DE PARCOURS dans un module partagé testable (`_shared/`), pas dans le handler edge — le handler ne fait que fournir un `fetchPage` et consommer le résultat. La fonction de parcours doit : (1) remonter toute page en échec comme une erreur explicite (`{ error }`), jamais l'avaler par un `break` silencieux, pour que le handler la transforme en réponse 5xx et donc en événement Sentry (règle [037]) ; (2) porter deux garde-fous, un plafond d'avis et un plafond de pages, car un jeton qui ne s'épuise jamais boucle jusqu'au timeout ; (3) marquer le résultat `truncated` quand un plafond est atteint ; (4) accepter la première page déjà récupérée (`firstPage`) pour ne pas la redemander — en mode itération à index gelé, un second appel repart sur un autre gel et le jeton ne correspond plus. Un compteur par cause de rejet (`kept`/`excluded`/`unmatched`/`unreadable`/`failed`) est renvoyé pour que chaque réglage se mesure au lieu de se supposer.
- **Vérification** : check [050] de `check-rules.sh` — tout fichier `supabase/functions/*-sync/index.ts` qui contient `iterationNextToken` ou `nextToken` ou `next_page` doit importer une fonction de parcours depuis `_shared/`, et le module de parcours correspondant doit avoir un fichier `.test.ts`. Contrôle manuel à la revue : la boucle ne doit pas contenir de `break` non suivi d'une remontée d'erreur.
- **Fichiers de référence** : `supabase/functions/_shared/ted.ts` (`walkTedPages`, `noticesOf`), `supabase/functions/_shared/ted.test.ts` (parcours, échec au milieu, plafonds, page déjà en main), `supabase/functions/ted-sync/index.ts`
- **Origine** : intégration TED — boucle de pagination non testée qui avalait les échecs, révélée par la demande explicite de « reprendre les tests et enrichir le harnais »
- **Date** : 2026-08-05

### [053] Historique d'un agent — append-only : le ménage se fait côté serveur, jamais en réécrivant des tours déjà envoyés
- **Constat** : 02/09/2026, lecture de l'annonce Claude Fable 5.1 et de sa règle « preserved thinking ». La signature d'un bloc `thinking` scelle tout ce qui le précède : system, tools, et chaque message antérieur. Réécrire un tour déjà envoyé invalide donc tous les blocs `thinking` postérieurs, et la requête part en 400 avant toute génération. `agent-chat` faisait exactement cela à deux endroits. (1) `compactForApi` rabotait les `tool_result` au-delà des 6 derniers messages : la frontière était figée pendant un tour, mais elle remontait à chaque nouveau tour utilisateur, donc un message envoyé entier au tour N était tronqué au tour N+1. (2) `summarizeIfLong` est une compaction « keep-tail » : elle remplace le début par un résumé et conserve la queue telle quelle, or les blocs `thinking` de cette queue ont été produits avec tout l'historique présent. Rien de tout cela n'était visible : l'organisation existante n'est pas encore soumise à la vérification, et Sonnet 5 l'ignore. La règle s'appliquera à tous les comptes sur les modèles suivants.
- **Règle** : Le tableau `messages` envoyé à l'API est append-only. Aucune troncature, aucun retrait d'image, aucune réécriture d'un tour déjà envoyé, et pas de reconstruction du `system` ni du tableau `tools` en cours de conversation. Le ménage de contexte passe par le **context editing serveur** (`clear_tool_uses_20250919`, header beta `context-management-2025-06-27`), qui ne compte pas comme une édition puisque l'API compare la conversation telle qu'elle a été envoyée ; `exclude_tools` protège les tools de lecture de contenu. Une compaction cliente par résumé reste possible mais oblige à retirer les blocs `thinking` des tours conservés en queue (`stripThinking`) : leur texte et leurs `tool_use` restent valides, pas leurs signatures. Une instruction qui change en cours de session s'ajoute en message `{role: "system"}`, elle ne modifie pas le `system` de tête. Poser, déplacer ou retirer un `cache_control` reste autorisé.
- **Vérification** : check [053] de `check-rules.sh` — `agent-chat` doit envoyer `clear_tool_uses_20250919` et le header `context-management-2025-06-27`, et aucun `compactForApi` ne doit réapparaître dans `supabase/functions/`. Test d'invariant `_shared/agent-history.test.ts` : plusieurs rounds rejoués, le préfixe déjà rendu doit être identique octet pour octet, `cache_control` mis à part. Garde-fou vérifié en le faisant échouer volontairement (règle [043]).
- **Fichiers de référence** : `supabase/functions/_shared/agent-history.ts` (`stripThinking`, `CONTENT_READ_TOOLS`, `withCacheBreakpoints`), `supabase/functions/agent-chat/index.ts` (`context_management`, `summarizeIfLong`), `supabase/functions/_shared/agent-history.test.ts`
- **Origine** : annonce Claude Fable 5.1 — audit de ce que la règle « preserved thinking » casserait dans la boucle d'agent
- **Date** : 2026-09-02
...
- **Vérification** : check [048] de `check-rules.sh` — `agent-chat` doit appeler `withCacheBreakpoints(conversationMessages)` et le test d'invariant `_shared/agent-history.test.ts` doit exister. Ce test rejoue plusieurs rounds et exige que le préfixe déjà rendu soit identique octet pour octet d'un round au suivant. Le cutoff figé qui portait cette garantie a été remplacé par la suppression pure et simple du rabotage client, voir règle [053]. Contrôle en production : dans Monitoring → Coûts API, les `cache_read` d'une conversation multi-rounds doivent être non nuls dès le round 2. S'ils restent à zéro, un invalidateur silencieux est revenu.
- **Fichiers de référence** : `supabase/functions/_shared/agent-history.ts` (`withCacheBreakpoints`), `supabase/functions/_shared/agent-history.test.ts`, `supabase/functions/agent-chat/index.ts`
...
- **Règle** : `refetchOnWindowFocus: false` doit être défini globalement dans le QueryClient. Ne jamais ajouter `refetchOnWindowFocus: true` sur des queries qui alimentent des formulaires. Les `useEffect` qui initialisent un formulaire depuis des données query doivent être gardés par un ID (ex: `entity.id !== prevIdRef.current`) pour ne pas écraser les modifications en cours lors d'un refetch. Exception étroite : une query en LECTURE SEULE qui n'alimente aucun formulaire (une liste qu'on veut fraîche au retour d'onglet, ex. la revue des marchés publics après une purge) peut réactiver `refetchOnWindowFocus: true`, à condition de le justifier par un commentaire `// safe:` sur la ligne — c'est ce que le check tolère, et rien d'autre.
- **Vérification** : check [006] de `check-rules.sh` — `grep "refetchOnWindowFocus: true" src/` hors lignes marquées `// safe:` doit être vide. Vérifier que le QueryClient dans `App.tsx` a bien `refetchOnWindowFocus: false`. Chercher `useEffect` + `setValues`/`setValue`/`setFormData` dépendant de données query sans garde par ID.
...
### [052] Réponse Anthropic — `content[0]` n'est pas le texte, et un refus n'est pas une erreur HTTP
- **Constat** : 02/09/2026, audit déclenché par l'annonce Claude Fable 5.1. Dix-sept lectures de réponse Anthropic dans `supabase/functions/` faisaient `data.content?.[0]?.text`. Or le thinking est adaptatif **par défaut** à partir de Sonnet 5 : sans paramètre `thinking`, le premier bloc renvoyé est un `thinking` (au texte vide, l'affichage étant `omitted` par défaut), et lire l'index 0 rend `undefined` sans lever d'erreur. Cinq de ces appels tournent déjà sur `CLAUDE_ADVANCED` (= `claude-sonnet-5`), et les douze autres basculeraient à la première modification de `_shared/claude-models.ts`, dont c'est précisément le rôle. Second angle mort : un refus de classifieur revient en **HTTP 200** avec `stop_reason: "refusal"` et aucun bloc texte — indiscernable d'une réponse vide, et invisible dans les coûts.
- **Règle** : Ne jamais lire une réponse Messages API par l'index d'un bloc. Passer par `_shared/anthropic-response.ts` : `anthropicText(data)` concatène les blocs `text` (il peut y en avoir plusieurs avec les citations) et rend `""` s'il n'y en a aucun ; `anthropicRefusal(data)` rend un message si `stop_reason` vaut `refusal`, sinon `null`. Le refus se teste **avant** la lecture du texte, et remonte en erreur : `aiChat` le trace alors dans `api_usage_events` au lieu de rendre une chaîne vide. Corollaire : un fallback en `?? "{}"` ne rattrape pas une chaîne vide — écrire `|| "{}"`.
- **Vérification** : check [052] de `check-rules.sh` — aucun `content[0]` ni `content?.[0]` dans `supabase/functions/`, hors `_shared/anthropic-response.ts` et fichiers de test. Tests unitaires dans `_shared/anthropic-response.test.ts`. Garde-fou vérifié en le faisant échouer volontairement (règle [043]).
- **Fichiers de référence** : `supabase/functions/_shared/anthropic-response.ts`, `supabase/functions/_shared/anthropic-response.test.ts`, `supabase/functions/_shared/ai.ts` (`callAnthropic`), `supabase/functions/_shared/claude-models.ts`
- **Origine** : annonce Claude Fable 5.1 — revue des appels Anthropic du repo à cette occasion
- **Date** : 2026-09-02
...
- **Vérification** : check [051] de `check-rules.sh` — `ted-sync` doit charger `tender_ted_cpv_codes` (liste CPV propre à la source) et ne doit plus référencer `tender_ted_languages` ni `isReadableLanguage` (filtre inerte retiré). Contrôle à la revue de tout nouveau connecteur multi-sources : tout réglage partagé doit être justifié par « même bonne valeur pour toutes les sources », sinon le scinder.
- **Fichiers de référence** : `supabase/functions/ted-sync/index.ts` (`CPV_SETTING`, `tedConfig`), `supabase/migrations/20260805081220_c03a84e8-fe6f-485e-93ee-a5aa2390917c.sql` (liste CPV propre au TED, filtre de langue supprimé), `supabase/functions/_shared/ted.ts` (en-tête : ni géographie ni langue ne filtrent)
- **Origine** : calibrage de la source TED — 248 avis pour une poignée attendue, tous tirés de codes CPV partagés inondant à l'échelle européenne, et filtre de langue inerte parce que le TED traduit tout
- **Date** : 2026-08-05
