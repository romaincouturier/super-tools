...
- **Règle** : Un catch qui se termine en toast sans capture est un angle mort d'observabilité. Trois points de passage obligatoires : (1) `toastError` capture via `Sentry.captureException` — passer l'erreur d'origine dans `options.cause` quand le message affiché est générique ; (2) le `QueryClient` garde un `onError` global sur `queryCache` et `mutationCache` ; (3) côté edge, `createErrorResponse` reporte les 5xx et les erreurs de quota IA (402, 429) via `reportEdgeError` — ne pas construire de réponse d'erreur 5xx à la main sans reporter. Ne jamais retirer ces branchements.
- **Vérification** : check [037] de check-rules.sh — `captureException` présent dans `src/lib/toastError.ts`, `queryCache`/`mutationCache` avec `onError` dans `src/App.tsx`, `reportEdgeError` appelé dans `createErrorResponse` de `supabase/functions/_shared/cors.ts`.
- **Fichiers de référence** : `src/lib/toastError.ts`, `src/App.tsx`, `supabase/functions/_shared/cors.ts`, `supabase/functions/_shared/sentry.ts`, `src/components/feedback/FeedbackForm.tsx` (premier consommateur de `options.cause`)
