...
- **Règle (contexte)** : capturer ne suffit pas — chaque événement doit porter QUI et OÙ. Front : `setSentryUser` (id, email, tag `role` staff/learner) posé par `useSentryInit` à chaque changement d'auth ; tag `module` posé par `PageViewTracker` à chaque navigation ; `release` = SHA du commit injecté au build (`VITE_COMMIT_SHA`, vite.config.ts) pour corréler erreurs et déploiements. Edge : `reportEdgeError` pose le tag `fn` (nom de la fonction, filtrable dans Sentry) ; les crons/batchs reportent chaque item en échec avec son `itemId`, jamais seulement un compteur agrégé.
- **Vérification** : checks [037], [037c], [037d], [037e] + ratchets [037a], [037b] de check-rules.sh.
