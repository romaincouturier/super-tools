...
- **Vérification** : check [053] de `check-rules.sh` — aucun `new Response(<corps non vide>, { status: 204 | 205 | 304 })` dans les fichiers de test. Au-delà du grep, la vérification manuelle est de relancer la couverture après chaque test ajouté et de confirmer que la ligne ciblée est passée au vert.
- **Fichiers de référence** : `supabase/functions/_shared/drive-resumable-upload.test.ts`, `supabase/functions/_shared/google-oauth.test.ts`
- **Origine** : test d'annulation de session Drive qui passait sans jamais exercer le chemin nominal
- **Date** : 2026-08-31

### [054] Toute edge function doit être déclarée dans `config.toml`
- **Constat** : 31/08/2026, audit de cohérence pendant `/sync-and-pr`. Trois dossiers de `supabase/functions/` n'apparaissaient pas dans `config.toml` : `ted-sync`, `tender-analyze`, `upload-tender-document`. Une fonction non déclarée se déploie avec les défauts de la CLI, dont `verify_jwt = true` — ce qui peut être le bon réglage, mais par accident : personne ne l'a décidé, et rien ne le dit. L'écart est invisible tant qu'on ne compare pas les deux listes à la main.
- **Règle** : Toute nouvelle edge function est déclarée dans `supabase/config.toml` dans le même commit que sa création, avec son `verify_jwt` explicite. Une fonction appelée depuis le frontend qui vérifie elle-même l'utilisateur (`verifyAuth(req)`) déclare `verify_jwt = false` ; une fonction sans contrôle interne garde `verify_jwt = true`. Quand on déclare après coup une fonction déjà en production, reprendre la valeur effective actuelle plutôt que la convention du voisin : changer le réglage en même temps qu'on le déclare, c'est modifier la sécurité d'une fonction sans que la revue le voie.
- **Vérification** : check [054] de `check-rules.sh` — chaque dossier de `supabase/functions/` hors `_shared` a une section `[functions.<nom>]` dans `config.toml`, et réciproquement.
- **Fichiers de référence** : `supabase/config.toml`
- **Origine** : audit `/sync-and-pr` sur la branche de sauvegarde des gros fichiers
- **Date** : 2026-08-31
...
### [052] Edge functions — le dialogue avec un service tiers et les tables de correspondance vivent en un seul exemplaire dans `_shared/`
- **Constat** : 31/08/2026, en débloquant la sauvegarde des gros fichiers storage. Le projet contenait **trois** tables extension → type MIME (`_shared/mime-types.ts`, `_shared/file-utils.ts`, une copie locale dans `scheduled-backup`) plus **six** copies de `resolveContentType()` dans les fonctions `upload-*`, et **cinq** copies du rafraîchissement de token OAuth Google (`scheduled-backup`, `backup-export`, `google-calendar-events`, `generate-certificates`, `_shared/google-drive-helper`). Ces copies avaient déjà divergé : seules certaines connaissaient `.mov`, si bien que les vidéos débloquées par le correctif seraient parties dans Drive en `application/octet-stream`, illisibles, sans aucune erreur. Les copies du refresh divergeaient aussi sur la gestion d'erreur — deux levaient, deux retournaient `null`, et aucune ne traitait le cas où Google répond 200 avec un corps d'erreur, qui donnait un token `undefined` et un échec sans cause identifiable plus loin.
- **Règle** : Deux catégories ne se recopient jamais d'une edge function à l'autre. (1) **Le protocole d'un service tiers** — échange de token, en-têtes d'authentification, format de requête : un seul module `_shared/` par service (`google-oauth.ts` pour le refresh Google, `drive-resumable-upload.ts` pour l'upload Drive). Un changement côté fournisseur doit se corriger à un seul endroit. (2) **Les tables de correspondance** — extension → MIME, code → libellé, statut → couleur : une valeur manquante dans une copie est un bug silencieux, jamais une erreur. Quand une fonction a besoin d'un comportement légèrement différent, elle passe un paramètre (`mimeTypeFromFileName(nom, fallback)`), elle ne recopie pas la table. Une liste d'extensions **autorisées** propre à un bucket reste locale : c'est une règle métier, pas une table de correspondance.
- **Vérification** : checks [052a] et [052b] de `check-rules.sh` — aucun `grant_type: "refresh_token"` hors de `_shared/google-oauth.ts`, et aucune table extension → MIME hors des deux fichiers de référence. Il y en a deux, et pas un seul, parce que le frontend (Vite) et les edge functions (Deno) ne peuvent pas importer le même module : `_shared/mime-types.ts` fait référence côté serveur, `src/lib/file-utils.ts` côté navigateur. Toute nouvelle entrée doit être ajoutée aux deux.
- **Fichiers de référence** : `supabase/functions/_shared/mime-types.ts`, `supabase/functions/_shared/google-oauth.ts`, `supabase/functions/_shared/file-utils.ts`, `supabase/functions/upload-crm-image/index.ts` (fallback spécifique passé en paramètre)
- **Origine** : sauvegarde des fichiers storage > 25 Mo — le type MIME des `.mov` manquait dans la seule copie qui servait
- **Date** : 2026-08-31
