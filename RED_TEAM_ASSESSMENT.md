...
- ~~**F4 ecriture**~~ CORRIGE : ecriture certificats restreinte au staff.
  - **Lecture publique certificats : vraisemblablement VOLONTAIRE, non fermee.** L'attestation est envoyee au participant (PDF joint a l'email) et `certificate_url` est stocke ; le chemin est un UUID `trainingId/fichier` non enumerable. Rendre le bucket prive casserait l'acces des participants a leur attestation Qualiopi (pas de session pour un participant anonyme). Recommandation : garder la lecture publique (equivalent d'un lien non devinable), l'ecriture etant deja restreinte au staff. A rouvrir seulement si une exigence de confidentialite l'impose (alors : signed URLs + acces authentifie participant).
...
- **D3 (MOYEN)** Injection de prompt indirecte : l'agent indexe `inbound_emails` / `crm_email` (contenu externe) et peut declencher `execute_action` (ecritures en service_role).
  - **Deja en place** : le system prompt d'`agent-chat` (ligne ~169) delimite explicitement le contenu recupere comme DONNEE, jamais des instructions, et demande d'ignorer + signaler tout contenu qui tente de rediriger l'agent. La separation donnees/instructions existe donc.
  - **Reste (decision produit)** : ajouter une confirmation humaine avant tout `execute_action` mutant. C'est un arbitrage autonomie/securite de l'agent (peut casser des workflows autonomes) : a decider, non applique.
...
- **Webhooks (comparaison timing) CORRIGE** : `assemblyai-webhook`, `fireflies-webhook`, `crm-elementor-webhook`, `supertilt-webhook` comparaient leur secret/signature avec `!==`/`===`. Passe en comparaison a temps constant via `_shared/crypto.ts:timingSafeEqualSecret` (compare les digests SHA-256). Supprime le canal auxiliaire temporel sans changer le schema d'auth (aucune reconfig fournisseur).
  - **Reste (config fournisseur, non applicable en code seul)** : HMAC du body + anti-rejeu (timestamp/nonce) pour assemblyai/fireflies. Necessite que le fournisseur signe le body (AssemblyAI n'envoie qu'un secret statique ; Fireflies supporte `x-hub-signature` mais exige une reconfig cote Fireflies). A activer cote fournisseur puis verifier le HMAC.
