...
// Combined scopes for Drive + Calendar + Search Console + Sheets
const SCOPES = [
...
function redirectToClientCallback(
  ...
  if (!appCallbackUrl) {
    return new Response("Authentification Google terminée. Vous pouvez fermer cette fenêtre.", {
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (action === "initiate") {
      ...
      const redirectUri = body.redirectUri || `${SUPABASE_URL}/functions/v1/google-auth?action=callback`;
    ...
    if (action === "status") {
      ...
      // Fallback: check legacy tables — if either is connected, report needsUpgrade
      const [driveResult, calendarResult] = await Promise.all([
        supabase.from("google_drive_tokens").select("id, token_expires_at").eq("user_id", userData.user.id).single(),
        supabase.from("google_calendar_tokens").select("id, token_expires_at").eq("user_id", userData.user.id).single(),
      ]);

      const hasLegacy = !!(driveResult.data || calendarResult.data);

      return new Response(JSON.stringify({
        connected: hasLegacy,
        needsUpgrade: hasLegacy,
        expiresAt: driveResult.data?.token_expires_at || calendarResult.data?.token_expires_at || null,
      }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    ...
    if (action === "disconnect") {
      ...
      await Promise.all([
        supabase.from("google_tokens").delete().eq("user_id", userData.user.id),
        supabase.from("google_drive_tokens").delete().eq("user_id", userData.user.id),
        supabase.from("google_calendar_tokens").delete().eq("user_id", userData.user.id),
      ]);
