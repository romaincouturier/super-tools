...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    let query = supabase.from("convention_signatures").select("*");
    if (signatureId) {
    ...
    if (sig.status === "signed" && sig.signed_at) {
      checks["signature_status"] = { status: "✅ CONFORME", detail: "Convention signée électroniquement" };
    } else {
      checks["signature_status"] = { status: "⚠️ EN ATTENTE", detail: `Statut: ${sig.status}` };
    }

    // 2. Verify PDF integrity
    if (sig.pdf_hash && sig.pdf_url) {
      try {
        ...
        if (pdfResponse.ok) {
          ...
          if (currentPdfHash === sig.pdf_hash) {
            checks["pdf_integrity"] = {
              status: "✅ CONFORME",
              detail: `Hash SHA-256 vérifié : ${sig.pdf_hash.substring(0, 16)}...`,
            };
          } else {
            checks["pdf_integrity"] = {
              status: "❌ NON CONFORME",
              detail: `Le PDF a été modifié après envoi. Hash original: ${sig.pdf_hash.substring(0, 16)}... / Hash actuel: ${currentPdfHash.substring(0, 16)}...`,
            };
          }
        } else {
      ...
      checks["pdf_integrity"] = { status: "⚠️ PARTIEL", detail: "Pas de hash PDF enregistré à la création" };
    }
    ...
    if (sig.proof_file_url && sig.proof_hash) {
      try {
        ...
        if (sig.proof_file_url.startsWith("signature-proofs/")) {
          // Private bucket - download via Supabase storage
        ...
        if (proofContent) {
          ...
          if (currentProofHash === sig.proof_hash) {
            ...
            checks["proof_integrity"] = {
              status: "❌ NON CONFORME",
              detail: `Le dossier de preuve a été altéré ! Hash original: ${sig.proof_hash.substring(0, 16)}... / Hash actuel: ${currentProofHash.substring(0, 16)}...`,
            };
          }
        } else {
    ...
    if (sig.confirmation_email_sent_at) {
      checks["confirmation_email"] = {
        status: "✅ CONFORME",
        detail: `Email de confirmation envoyé le ${sig.confirmation_email_sent_at}`,
      };
    } else {
      checks["confirmation_email"] = { status: "⚠️ ABSENT", detail: "Pas d'email de confirmation envoyé" };
    }
    ...
    await reportEdgeError(error, { fn: "verify-convention-signature" });
