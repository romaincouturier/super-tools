...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Link to the onboarding page (account creation / login)
    const onboardingUrl = `${appUrl}/apprenant/connexion?token=${link.token}`;
    const firstName = participants[0].first_name || "Apprenant";

    await sendEmail({
      to: email,
      subject: "Votre accès à votre formation en ligne",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; padding: 20px;">
          <h1 style="color: #101820; font-size: 24px;">Bonjour ${firstName} 👋</h1>
          <p style="color: #555; font-size: 16px; line-height: 1.6;">
            Votre accès à votre formation en ligne a été activé. Cliquez sur le bouton ci-dessous pour créer votre compte et accéder à votre formation :
          </p>
