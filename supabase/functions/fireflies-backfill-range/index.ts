...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    for (let page = 0; page < maxPages; page++) {
      ...
      for (const t of list) {
        ...
        try {
          ...
          totals.errors++;
          await reportEdgeError(e, { fn: "fireflies-backfill-range", itemId: t.id });
    ...
    console.error("[fireflies-backfill-range] error:", error);
    await reportEdgeError(error, { fn: "fireflies-backfill-range" });
    const message = error instanceof Error ? error.message : String(error);
    return new Response(JSON.stringify({ ok: false, error: message, totals, log }), {
      status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
