...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const filePath = `${trainingId}/participant_${participantId}/convention_signee_${Date.now()}_${sanitizedName}.${fileExt}`;
    ...
    if (uploadError) {
      console.error("[upload-participant-convention] storage error", uploadError);
    ...
    if (updateError) {
      console.error("[upload-participant-convention] db error", updateError);
    ...
    console.error("[upload-participant-convention] unexpected error", error);
    return new Response(
