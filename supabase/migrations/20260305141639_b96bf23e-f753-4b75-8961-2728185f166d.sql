...
-- Drop and recreate with formula-level filtering
...
BEGIN
  ...
  -- Find the specific formula AND its catalog
  SELECT ff.id AS formula_id, fc.id AS catalog_id
  INTO v_formula
  ...
  IF v_formula.catalog_id IS NULL THEN
    RETURN jsonb_build_object('status', 'product_not_found');
  END IF;

  -- Find participant matching email + catalog + specific formula
  ...
  WHERE lower(tp.email) = lower(p_email)
    AND t.catalog_id = v_formula.catalog_id
    AND tp.formula_id = v_formula.formula_id
  ...
  IF v_participant.id IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'catalog_id', v_formula.catalog_id
    );
  END IF;
  ...
  IF p_form_type = 'besoins' THEN
    ...
    WHERE participant_id = v_participant.id AND training_id = v_participant.training_id;
