-- Démolition du mécanisme de lien magique (plus aucune ouverture de session
-- automatique, nulle part). Édite d'abord les fonctions charnières qui
-- référencent learner_magic_links sans être elles-mêmes spécifiques au lien
-- magique (le corps plpgsql n'est pas vérifié à la création : les laisser
-- inchangées casserait ces fonctions au premier appel, une fois la table
-- supprimée), puis retire la table et les fonctions qui n'existaient que
-- pour elle.

-- resolve_login_identity : 5 états → 3. Un compte connu répond toujours
-- "password" (qu'il ait déjà un mot de passe ou non — "mot de passe oublié"
-- reste l'unique porte de recours). Un participant/inscrit sans compte
-- rejoint désormais "unknown" : plus de lien d'activation séparé.
...
-- get_learner_email : retire la branche learner_magic_links, garde les deux
-- autres (training_participants, lms_enrollments) et l'en-tête x-learner-
-- email tel quel — sa fermeture est un chantier séparé, déjà préparé dans
-- supabase/migrations-apres-front/20260915120000_lot6c_fermeture_entete_apprenant.sql,
-- pas l'objet de cette PR.
...
-- change_learner_email : ne pousse plus used_at sur des jetons qui n'existent
-- plus.
...
-- connexion_indicators : retire les compteurs de liens (links_sent,
-- links_expired_unused), qui n'ont plus de sens.
...
-- current_user_access_level : retire la branche learner_magic_links.
...
-- Plus aucune fonction ni policy ne référence learner_magic_links : la table
-- et son propre triplet de fonctions (supersedé par resolve-login-identity /
-- send-learner-access-email depuis les PR précédentes) peuvent disparaître.
