-- Lot 1 de la refonte de connexion apprenant.
-- Référence : docs/SPEC_CONNEXION_APPRENANT.md chapitre 21, docs/AUDIT_SURFACES_EXPOSEES.md.
-- Ferme deux trajectoires d'accès aux données d'un apprenant tiers :
--   1. l'en-tête x-learner-email primait sur le jeton dans get_learner_email()
--   2. get_learner_portal_data acceptait une adresse arbitraire d'un appelant anonyme
-- Aucun changement visible pour un apprenant légitime.

-- 1. Identité apprenant : le jeton d'abord, l'en-tête seulement à défaut de session.
--    Un apprenant authentifié ne peut plus se faire passer pour un autre en posant
--    l'en-tête. Le parcours historique sans compte reste servi par l'en-tête tant que
--    tous les apprenants n'ont pas de compte (lot 4).
...
-- 2. Données du portail : l'appelant ne peut demander que sa propre adresse.
...
DECLARE
  v_caller text;
BEGIN
  -- Lot 1 : l'appelant ne peut demander que sa propre adresse.
  -- Le staff (ligne dans profiles) conserve la prévisualisation.
