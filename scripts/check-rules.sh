...
if [ "$STAGED_MODE" = "true" ]; then
  ...
  EXEMPT_PAGES="Auth|Signup|ResetPassword|ForcePasswordChange|LearnerResetPassword|Landing|PolitiqueConfidentialite|Emargement|Evaluation|Questionnaire|TrainerEvaluation|SponsorEvaluation|ReclamationPublic|LearnerPortal|LearnerAccess|LearnerOnboarding|LearnerOnboarding.test|LmsCoursePlayer|LessonBuilderPage|NotFound|FormulaireRedirect|Screenshots|AgentChat|ArenaDiscussion|ArenaSetup|ArenaResults|Dashboard|FormationDetail|MissionSummary|TrainingSummary|TrainingSupportPage|Index|Onboarding|SignatureConvention|SignatureDevis|GoogleDriveCallback|LmsCourseEntry|PartnerPortal"
