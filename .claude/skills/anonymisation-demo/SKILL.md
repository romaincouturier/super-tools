...
| `scripts/check-demo-mask.sh` | Le contrôle. Règle [062], branchée en ratchet dans `scripts/check-rules.sh`. |
...
`scripts/rules-ratchet.txt` porte `062=<n>`. Le compte ne peut que descendre :
...
> `bash scripts/check-rules.sh`, abaisse le ratchet `062` et rends-moi la liste
