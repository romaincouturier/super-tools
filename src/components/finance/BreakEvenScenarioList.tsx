            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-muted-foreground">
                  Point mort : {Number.isFinite(m.pointMortRevenue) ? (isDemoMode ? maskAmount(m.pointMortRevenue) : EUR.format(m.pointMortRevenue)) : "—"} • Marge :{" "}
                  {isDemoMode ? maskAmount(m.monthlyMargin) : EUR.format(m.monthlyMargin)}
                </div>
              </div>
            ...
            return (
              <Card key={s.id}>
                ...
                <CardContent className="space-y-1 text-xs">
                  <div>Charges fixes : {isDemoMode ? maskAmount(Number(s.fixed_costs)) : EUR.format(Number(s.fixed_costs))}</div>
                  <div>Taux variable : {Math.round(Number(s.variable_cost_rate) * 100)}%</div>
                  <div>Prix moyen : {isDemoMode ? maskAmount(Number(s.avg_unit_price)) : EUR.format(Number(s.avg_unit_price))}</div>
                  <div>Volume / mois : {Number(s.monthly_units)}</div>
                  <div className="pt-1 border-t">
                    <span className="text-muted-foreground">Point mort : </span>
                    <strong>
                      {Number.isFinite(m.pointMortRevenue) ? (isDemoMode ? maskAmount(m.pointMortRevenue) : EUR.format(m.pointMortRevenue)) : "—"}
                    </strong>
                  </div>
                  <div>
