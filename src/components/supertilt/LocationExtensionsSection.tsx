...
function ExtensionRow({ ext, orderItemId }: { ext: LocationExtension; orderItemId: string }) {
  ...
  const signed = ext.signature_status === "signed";
    ...
    <div className="text-xs space-y-1 rounded border border-orange-200 p-1.5">
      ...
      {signed ? (
        <p className="flex items-center gap-1 text-green-700">
          ...
          Signé le {ext.signed_at ? day(ext.signed_at) : ""}
          {ext.signed_pdf_url && (
            <button type="button" onClick={() => openStorageUrl(ext.signed_pdf_url!)} className="ml-1 underline">
              Télécharger
            </button>
          )}
        </p>
      ) : ext.signature_sent_at ? (
        <p className="flex items-center gap-1 text-blue-700">
          <Send className="h-3 w-3" />
          En attente de signature (envoyé le {day(ext.signature_sent_at)})
        </p>
      ) : null}
