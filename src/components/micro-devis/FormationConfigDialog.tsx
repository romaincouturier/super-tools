import { Loader2, Save, X, Plus, Trash2, Star, ChevronUp, ChevronDown, Settings } from "lucide-react";
...
import type { FormationConfig } from "@/types/micro-devis";

interface FormationConfigDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  configs: FormationConfig[];
  loadingConfigs: boolean;
  editingFormation: FormationConfig | null;
  onEditFormation: (f: FormationConfig | null) => void;
  newFormation: Partial<FormationConfig> | null;
  onNewFormation: (f: Partial<FormationConfig> | null) => void;
  onSave: () => void;
  onAdd: () => void;
...
export function FormationConfigDialog({
  open,
  onOpenChange,
  configs,
  loadingConfigs,
          ...
          {newFormation ? (
            <div className="space-y-3 p-4 border rounded-lg bg-primary/5">
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" variant="outline" onClick={() => onNewFormation(null)}>
            ...
            <Button
              variant="outline"
              className="w-full"
              onClick={() => onNewFormation({ prix: 1490, duree_heures: 14 })}
            >
                ...
                {editingFormation?.id === config.id ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => onEditFormation(null)}>
                        <X className="w-3 h-3 mr-1" />
                        Annuler
                      </Button>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => onEditFormation(config)}
                    >
                      Modifier
                    </Button>
                  </div>
