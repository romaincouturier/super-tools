...
const CatalogFormDialog = ({ open, onClose, entry, onDelete, trainingCount = 0 }: CatalogFormDialogProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleClose(!!entry)}>
      <DialogContent className="w-full sm:max-w-2xl max-h-[90vh] overflow-y-auto overflow-x-hidden" onPointerDownOutside={(e) => e.preventDefault()} onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle>
