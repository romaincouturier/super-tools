...
import { KeyResultCard } from "./OKRKeyResultCard";
import { KeyResultDialog } from "./OKRKeyResultDialog";
import { CheckInDialog } from "./OKRCheckInDialog";
import { ParticipantRow, ParticipantDialog } from "./OKRParticipantSection";
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-4">
              ...
              <div>
                ...
                {objective.next_review_date && (
                  <p className="text-sm text-muted-foreground">
                    Prochain suivi:{" "}
                    {format(new Date(objective.next_review_date), "d MMMM yyyy", { locale: fr })}
                  </p>
                )}
              </div>
          ...
          <TabsContent value="team" className="mt-4 space-y-4">
            ...
            {participants && participants.length > 0 && (
              <div className="space-y-2">
                {participants.map((p) => (
                  <ParticipantRow key={p.id} participant={p} objectiveId={objective.id} />
                ))}
              </div>
            )}
