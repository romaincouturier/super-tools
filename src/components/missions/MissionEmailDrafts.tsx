...
export default function MissionEmailDrafts({ missionId }: Props) {
  const { drafts, isLoading, approveAndSend, rejectDraft, updateDraftContent, scheduleDraft } = useMissionEmailDrafts(missionId);
  ...
  if (drafts.length === 0) {
    return (
      <p className="text-sm text-muted-foreground text-center py-6">
        Aucun brouillon d'email pour cette mission.
      </p>
    );
  }
      ...
      {otherDrafts.length > 0 && (
        <>
          ...
          {otherDrafts.map((draft) => (
            <DraftCard key={draft.id} draft={draft} isMutating={false} />
          ))}
        </>
      )}
    </div>
...
function DraftCard({
  ...
  const handleSaveEdit = () => {
    if (onUpdateContent) {
      onUpdateContent(draft.id, editSubject, editHtml);
    }
    setEditing(false);
  };
