...
export async function createPage(input: { mission_id: string; parent_page_id?: string | null; title?: string; content?: string; activity_id?: string | null; icon?: string; page_type?: string; source_transcript_id?: string | null }): Promise<MissionPage> {
  ...
  const result = await db()
    ...
    .insert({
      ...
      position: maxPos + 1,
      ...(input.source_transcript_id ? { source_transcript_id: input.source_transcript_id } : {}),
