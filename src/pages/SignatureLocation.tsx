...
import { CheckCircle2, FileText, Package, User, PenLine, Shield, ExternalLink } from "lucide-react";
...
interface LocationSignatureData {
  id: string;
  token: string;
...
const SignatureLocation = () => {
  ...
  const [contractData, setContractData] = useState<LocationSignatureData | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const { data: signatureJson, error: sigError } = await rpc.getLocationSignatureByToken(token);
        ...
        if (signature.status === "signed" || signature.signed_at) {
          ...
          setError("Ce contrat a été annulé");
          setLoading(false);
          return;
        }
        ...
        if (!signature.email_opened_at) {
          await rpc.markLocationSignatureOpened(token, new Date().toISOString());
          trackEvent("first_link_opened");
        } else {
          trackEvent("link_reopened");
        }

        setContractData(signature);
        ...
        if (isS3PresignedUrl(signature.pdf_url)) {
          try {
            const refreshResult = await supabase.functions.invoke("refresh-convention-pdf-url", {
              body: { token, type: "location" },
            });
            if (refreshResult.data?.pdf_url && refreshResult.data.pdf_url !== signature.pdf_url) {
              setContractData((prev) => prev ? { ...prev, pdf_url: refreshResult.data.pdf_url } : prev);
            }
          } catch (refreshErr) {
  ...
  const handlePdfConsulted = () => {
    trackEvent("pdf_consulted", { pdf_url: contractData?.pdf_url });
  };
  ...
  const handleSubmit = async () => {
    ...
    if (result !== null) {
      ...
      toast({ title: "Contrat signé", description: "Votre signature a été enregistrée avec succès." });
    }
    setSubmitting(false);
  };
          ...
          <CardContent className="pt-6 text-center space-y-4">
            ...
            {contractData && (
              <div className="mt-4 text-sm text-muted-foreground space-y-1">
                <p><strong>{contractData.game_name}</strong></p>
                {contractData.contrat_reference && <p>Réf. {contractData.contrat_reference}</p>}
              </div>
            )}
    ...
    <div className="min-h-screen bg-background py-8 px-4">
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold">Signature du contrat de location</h1>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <span className="font-medium">{contractData?.game_name}</span>
            </div>
            {contractData?.contrat_reference && (
              <div className="flex items-center gap-3">
                <FileText className="h-4 w-4 text-muted-foreground" />
                <span className="text-sm text-muted-foreground">Réf. {contractData.contrat_reference}</span>
              </div>
            )}
            <div className="flex items-center gap-3">
              <FileText className="h-4 w-4 text-muted-foreground" />
              <span className="text-sm text-muted-foreground">
                Contrat du {contractData && formatCreatedDate(contractData.created_at)}
              </span>
            </div>
            {contractData?.pdf_url && (
              <div className="pt-2">
                <Button variant="outline" asChild className="w-full sm:w-auto" onClick={handlePdfConsulted}>
                  <a href={contractData.pdf_url} target="_blank" rel="noopener noreferrer">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="signerFunction" value={signerFunction} onChange={(e) => setSignerFunction(e.target.value)} placeholder="Ex: Directeur" />
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-start space-x-3 p-3 bg-muted/50 rounded-lg">
              ...
              <Label htmlFor="consent" className="text-sm leading-relaxed cursor-pointer">
                En signant ce contrat de location, j'accepte les conditions générales de location et je
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleSubmit} disabled={submitting || !consentGiven || !signerName.trim()} className="flex-1">
                {submitting ? (<><Spinner className="mr-2" />Envoi...</>) : "Signer le contrat"}
              </Button>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-4">
            <div className="flex items-start gap-3">
              ...
              <div className="text-xs text-muted-foreground space-y-2">
                ...
                <p>Données enregistrées : votre signature, votre nom, la date et l'heure, votre adresse IP, les informations de votre appareil, et l'intégralité de votre parcours de signature.</p>
              </div>
            </div>
          </CardContent>
        </Card>
