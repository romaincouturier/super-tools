...
export default function LmsCoursePlayer() {
  ...
  // Intercept clicks on internal lesson links inside lesson HTML.
  // Internal links generated by the link picker look like:
  //   /lms/{courseId}/player?lesson={lessonId}
  // For learners we must preserve the ?email= param (lost otherwise) and,
  // for same-course links, swap the lesson in place instead of full reload.
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (playerMatch) {
        targetCourseId = playerMatch[1];
        const qp = new URLSearchParams(playerMatch[2] || "");
        targetLessonId = qp.get("lesson");
      } else if (builderMatch) {
        targetCourseId = builderMatch[1];
        targetLessonId = builderMatch[2];
      } else {
        return;
      }
      e.preventDefault();
      if (targetCourseId === courseId && targetLessonId) {
        // Same course: swap lesson without reload, preserve current URL/query state
        setSelectedLessonId(targetLessonId);
        const qp = new URLSearchParams(window.location.search);
        qp.set("lesson", targetLessonId);
        navigate(`/lms/${courseId}/player?${qp.toString()}`, { replace: false });
      } else if (targetCourseId) {
        // Cross-course: preserve email / preview flags
        const qp = new URLSearchParams();
        if (learnerEmail) qp.set("email", learnerEmail);
        if (isPreview) qp.set("preview", "admin");
        if (targetLessonId) qp.set("lesson", targetLessonId);
        navigate(`/lms/${targetCourseId}/player?${qp.toString()}`);
      }
    };
