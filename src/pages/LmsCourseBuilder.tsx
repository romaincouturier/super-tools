...
function CourseSettings({ course }: { course: { id: string; title: string; description: string | null; difficulty_level: string | null; estimated_duration_minutes: number; welcome_video_url?: string | null; welcome_text?: string | null; home_config?: CourseHomeConfig | null } }) {
  const updateCourse = useUpdateCourse();
  ...
  const handleSave = async () => {
    await updateCourse.mutateAsync({ id: course.id, ...form, home_config: home });
    ...
    <Card>
      <CardContent className="space-y-4 pt-6">
        ...
        <div className="pt-4 border-t space-y-4">
          <div>
            <h3 className="font-semibold text-sm">Page d'accueil — infos pratiques</h3>
            <p className="text-xs text-muted-foreground">Ces blocs n'apparaissent sur la page d'accueil que s'ils sont renseignés.</p>
          </div>
