...
export default function Landing() {
  ...
  const freeCourses = useMemo(() => {
    ...
    return gratuit.slice().sort((a, b) => rank(a.id) - rank(b.id));
  }, [courses]);
  ...
  return <main className="min-h-screen bg-background text-foreground">
    <nav className="absolute inset-x-0 top-0 z-20"><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-10"><Link to="/" aria-label="SuperTilt Academy"><SupertiltLogo className="h-9" /></Link><div className="hidden items-center gap-8 md:flex"><a href="#formations" className="text-sm font-semibold text-foreground underline-offset-4 hover:text-foreground hover:underline focus-visible:text-foreground focus-visible:underline">Toutes les formations</a><Link to="/connexion" className="text-sm font-semibold text-foreground underline-offset-4 hover:text-foreground hover:underline focus-visible:text-foreground focus-visible:underline">Se connecter</Link></div><button type="button" className="md:hidden" aria-label={menuOpen ? "Fermer le menu" : "Ouvrir le menu"} onClick={() => setMenuOpen((open) => !open)}>{menuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}</button></div>{menuOpen && <div className="border-y border-border bg-background px-6 py-4 md:hidden"><div className="flex flex-col gap-4 text-sm font-semibold"><a href="#formations" onClick={() => setMenuOpen(false)}>Toutes les formations</a><Link to="/connexion">Se connecter</Link></div></div>}</nav>

    <section className="relative overflow-hidden bg-primary px-6 pb-20 pt-32 lg:px-10 lg:pb-28 lg:pt-44"><div className="pointer-events-none absolute -right-20 top-24 h-64 w-64 rounded-full border-[28px] border-primary-foreground/10 lg:h-96 lg:w-96" /><div className="pointer-events-none absolute bottom-[-180px] left-[8%] h-72 w-72 rounded-full border-[18px] border-primary-foreground/10" /><div className="relative mx-auto max-w-7xl"><div><p className="mb-7 text-sm font-bold uppercase tracking-[0.18em] text-primary-foreground/70">SuperTilt Academy</p><h1 className="text-5xl font-black leading-[1.02] tracking-tight text-primary-foreground sm:text-6xl lg:text-7xl xl:text-8xl">Apprenez, expérimentez, progressez avec SuperTilt.</h1><p className="mt-8 max-w-3xl text-lg leading-8 text-primary-foreground/80 sm:text-xl">Des formations en ligne, des mini-cours et des ressources pour progresser en facilitation graphique, intelligence collective et gestion de projet. Et pour commencer, plusieurs formations sont accessibles gratuitement.</p><div className="mt-10 flex flex-col gap-3 sm:flex-row"><Button asChild size="lg" className="bg-foreground px-7 font-bold text-background hover:bg-foreground/90"><a href="#formations">Voir les formations gratuites <ArrowRight className="ml-2 h-5 w-5" /></a></Button></div></div></div></section>

    <section id="formations" className="bg-background px-6 py-20 lg:px-10 lg:py-28"><div className="mx-auto max-w-7xl"><div className="mb-12 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="mb-3 text-sm font-bold uppercase tracking-[0.15em] text-primary">Pour commencer</p><h2 className="text-4xl font-black tracking-tight sm:text-5xl">Commencez gratuitement</h2></div><p className="max-w-xl text-base leading-7 text-muted-foreground">Envie de tester l’Academy ? Commencez avec l’une de nos formations gratuites. Créez simplement votre compte et lancez-vous.</p></div>{isLoading ? <div className="flex min-h-40 items-center justify-center"><Spinner size="lg" className="text-primary" /></div> : isError ? <p className="border border-border bg-secondary p-6 text-muted-foreground">Les formations seront bientôt disponibles.</p> : freeCourses.length > 0 ? <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{freeCourses.map((course, index) => <FreeCourseCard key={course.id} course={course} index={index} />)}</div> : <p className="text-muted-foreground">Aucune formation gratuite n’est disponible pour le moment.</p>}</div></section>

    <section id="expertises" className="bg-secondary px-6 py-20 lg:px-10 lg:py-28"><div className="mx-auto max-w-7xl"><div className="mb-12 flex items-end justify-between gap-8"><div><p className="mb-3 text-sm font-bold uppercase tracking-[0.15em] text-primary">Expertises</p><h2 className="text-4xl font-black tracking-tight sm:text-5xl">Ce que vous pouvez apprendre</h2></div><Compass className="hidden h-14 w-14 text-primary md:block" strokeWidth={1.5} /></div><div className="grid border-t border-border md:grid-cols-3">{expertise.map((item) => <article key={item.label} className="border-b border-border py-8 md:border-b-0 md:border-r md:px-8 md:first:pl-0 md:last:border-r-0"><span className="text-sm font-black text-primary">{item.mark}</span><h3 className="mt-10 text-2xl font-bold leading-tight">{item.label}</h3><p className="mt-4 text-base leading-7 text-muted-foreground">{item.text}</p></article>)}</div></div></section>
