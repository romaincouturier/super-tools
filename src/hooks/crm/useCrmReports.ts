...
import { buildWeeklyTimeSeries } from "@/lib/crmWeeklySeries";

export type { WeeklyPoint } from "@/lib/crmWeeklySeries";
...
export const useCrmReports = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Apply date filter — WON/LOST use their temporal fields (fallback
      // created_at when the timestamp is missing), OPEN uses created_at
      const inPeriod = (card: CrmCard) => {
        if (!filters?.startDate && !filters?.endDate) return true;
        const refDate = card.sales_status === "WON" ? (card.won_at ?? card.created_at)
          : card.sales_status === "LOST" ? (card.lost_at ?? card.created_at)
          : card.created_at;
