...
import { logCrmActivity } from "@/data/crm";
import { CRM_QUERY_KEY } from "@/hooks/queries/useCrmQueries";
...
export const useCreateCard = () => {
  ...
  return useMutation({
    mutationFn: async ({ input, actorEmail }: { input: CreateCardInput; actorEmail: string }) => {
      ...
      const { data, error } = await supabase.from("crm_cards").insert(insertData).select().single();
    ...
    mutationFn: async ({
      ...
      if (updates.sales_status && updates.sales_status !== oldCard.sales_status) {
        await logActivity(
          ...
          actorEmail,
          oldCard.sales_status,
          updates.sales_status,
        );
      }
      if (
        updates.estimated_value !== undefined &&
        updates.estimated_value !== oldCard.estimated_value
      ) {
        await logActivity(
          id,
